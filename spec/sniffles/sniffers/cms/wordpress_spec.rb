...
describe Sniffles::Sniffers::Wordpress do
  describe "#output" do
    context "feed, theme, pingback", :vcr => { :cassette_name => 'pearsonified_com' } do
      let(:wp) { Sniffles::Sniffers::Wordpress.new(page_body("http://www.pearsonified.com/"))  }
    ...
    context "version", :vcr => { :cassette_name => 'humemes_com' } do
      let(:wp) { Sniffles::Sniffers::Wordpress.new(page_body("http://humemes.com/")) }
      
      it { wp.output[:version].should eq "2.3.1" }
    end  
    
    context "not wordpress" do
      let(:blank) { Sniffles::Sniffers::Wordpress.new(empty_html_doc) }
